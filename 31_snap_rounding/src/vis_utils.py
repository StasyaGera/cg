...
def draw_all(segments, status, events, hot, current):
	...
	seg_src = list(map(lambda seg: [(seg.start.x, seg.start.y), (seg.end.x, seg.end.y)], status))
	...
	for s in segments:
		if s not in status:
			seg_col.append('blue')
			seg_src.append(s)
	seg_src = list(map(lambda seg: [(seg.start.x, seg.start.y), (seg.end.x, seg.end.y)], seg_src))
	ax.add_collection(mc.LineCollection(seg_src, colors=seg_col))

	for p in current:
		ax.add_patch(patches.Rectangle((p.sw.x, p.sw.y), eps, eps, alpha=0.5, facecolor="yellow"))

	for p in hot:
		ax.add_patch(patches.Rectangle((p.sw.x, p.sw.y), eps, eps, alpha=0.5, facecolor="red"))

	e_src = defaultdict(list)
	for e in events:
		e_src[_type_color(e)].append(e)
	for t in e_src:
		plt.plot([e.x for e in e_src[t]], [e.y for e in e_src[t]], t, label=e_src[t][0].etype.name, zorder=1)
